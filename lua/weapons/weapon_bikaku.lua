...
local tr = util.TraceLine( {
		...
		local dmg = math.random(BIKAKUPRMATK1* ( ply:GetNWInt("RCells", GHOULLIMITRC) / GHOULLIMITRC ),BIKAKUPRMATK2 * ( ply:GetNWInt("RCells", GHOULLIMITRC) / GHOULLIMITRC ) )
...
function SWEP:SecondaryAttack()
	...
	if ply:GetNWInt("ARCGaz", 0 ) > BIKAKUARCGazSCRATK or ply:GetNWInt("TKGFood") < KAGUNEMINFOOD then
		return
	end
...
local tr = util.TraceLine( {
		...
		local dmg = math.random(BIKAKUSCRATK1* ( ply:GetNWInt("RCells", GHOULLIMITRC) / GHOULLIMITRC ),BIKAKUSCRATK2 * ( ply:GetNWInt("RCells", GHOULLIMITRC) / GHOULLIMITRC ) )
	...
	if ( SERVER && IsValid( tr.Entity ) ) then
		local phys = tr.Entity:GetPhysicsObject()
		if ( IsValid( phys ) ) then
			phys:ApplyForceOffset( self.Owner:GetAimVector() *( dmg * dmgr * 750)* phys:GetMass() , tr.HitPos )
		end
	end
end
