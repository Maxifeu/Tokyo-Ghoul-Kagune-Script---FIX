...
function ENT:Initialize()
	...
	self:SetColor( Color( 128, 0, 100, 255 ) )
	...
	if ent:IsNPC() or ent:IsPlayer() or ent:IsWorld() or ent:IsVehicle() or ent:IsValid() then
		...
		if speed < 150 then
			...
			if speed >= 150 then
				...
				self:SetParent(ent)
				ent:TakeDamage(math.random(10, 15) , self:GetOwner(), self)
		...
		if ent:IsWorld() then
			...
			if ent:IsValid() then
				...
				self:PhysicsInit( SOLID_NONE )
				self:SetParent(ent)
				ent:TakeDamage(math.random(QKQRPRMATK1, QKQRPRMATK2) * self:GetNWInt("Damage",1.25), self:GetOwner(), self)
				local physforce = speed * math.random(QKQRPRMATK1*2, QKQRPRMATK2*1.5)
